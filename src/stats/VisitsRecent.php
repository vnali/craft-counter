...
class VisitsRecent extends Date
{
    ...
    public function getData(): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($this->showChart) {
                ...
                $dateKey = CounterDateTimeHelper::intlDate($datetime, null, CounterDateTimeHelper::intlFormat($interval));
                ...
                if ($this->showVisitor) {
                    ...
                    if ($this->preferredInterval == 'hourly') {
                        if (!isset($data[$dateKey]['visitors'])) {
                            ...
                            $data[$dateKey]['visitors'] = (int)count(array_unique($query->column()));
                        }
                    } elseif ($this->preferredInterval == '30minutes') {
                        $dateKeyParts = explode(':', $dateKey);
                        if ($dateKeyParts[1] == '0') {
        ...
        if ($this->showChart) {
            ...
            $data = CounterDateTimeHelper::fillTimes($data, ['visits', 'visitors', 'dateCreated'], $interval, $startDate, $endDate, $this->dateRange, null, $this->preferredInterval);
